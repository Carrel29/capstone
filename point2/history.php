...
$paymentFilter = isset($_GET['payment_method']) ? $_GET['payment_method'] : 'all';
$startDate = isset($_GET['start_date']) ? $_GET['start_date'] : '';
$endDate = isset($_GET['end_date']) ? $_GET['end_date'] : '';

// Build the base query - using payment_code for JOIN
$query = "SELECT t.*, pm.name as payment_name, pm.code as payment_code 
          FROM transactions t
          LEFT JOIN payment_methods pm ON t.payment_code = pm.code
          WHERE 1=1";

// Payment method filter - now works with any payment code
if ($paymentFilter !== 'all') {
    $query .= " AND pm.code = '$paymentFilter'";
}

// Time filter
switch($timeFilter) {
    case 'hour':
        $query .= " AND t.transaction_date >= DATE_SUB(NOW(), INTERVAL 1 HOUR)";
        break;
    case '24hours':
        $query .= " AND t.transaction_date >= DATE_SUB(NOW(), INTERVAL 24 HOUR)";
        break;
    case 'week':
    ...
    case 'custom':
        if ($startDate && $endDate) {
            $query .= " AND DATE(t.transaction_date) BETWEEN '$startDate' AND '$endDate'";
        }
        break;
}
...
<body>
        ...
        <div class="filter-labels">
            ...
            <div class="filter-controls">
                ...
                <select name="payment_method">
                    ...
                    <?php
                    // Fetch all payment methods for the filter dropdown
                    $paymentMethods = $conn->query("SELECT code, name FROM payment_methods WHERE archived = 0 ORDER BY name");
                    while($pm = $paymentMethods->fetch_assoc()): ?>
                        <option value="<?php echo $pm['code']; ?>" <?php echo $paymentFilter == $pm['code'] ? 'selected' : ''; ?>>
    ...
    <td><?php echo htmlspecialchars($transaction['payment_name'] ?? ''); ?></td>
    <td>₱<?php echo number_format($transaction['total_amount'], 2); ?></td>
    <td>
        <?php
        if ($transaction['payment_code'] == 'CASH') {
    ...
    function viewDetails(transaction) {
        ...
        let detailsHtml = `
            ...
            <p><strong>Payment Method:</strong> ${transaction.payment_name || ''}</p>
            <p><strong>Total Amount:</strong> ₱${parseFloat(transaction.total_amount || 0).toFixed(2)}</p>
        `;

        if (transaction.payment_code === 'CASH') {
            detailsHtml += `
                <p><strong>Cash Received:</strong> ₱${parseFloat(transaction.cash_received || 0).toFixed(2)}</p>
                <p><strong>Change:</strong> ₱${parseFloat(transaction.cash_change || 0).toFixed(2)}</p>
            `;
        } else {
        ...
        for (const item of cartItems) {
            detailsHtml += `<li>${item.name || ''} x ${item.quantity || 0} - ₱${((item.price || 0) * (item.quantity || 0)).toFixed(2)}</li>`;
        }
        detailsHtml += `</ul>`;
