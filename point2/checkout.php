...
<body>
<div class="top-nav">
    <button class="back-to-pos-btn" onclick="goBackToPOS(false)">Back to POS</button>
</div>
...
<div id="messageModal" class="modal">
    <div class="modal-content" id="messageModalContent">
        ...
        <div class="modal-buttons">
            <button class="modal-button modal-button-primary" id="modalButton" onclick="closeModal()">OK</button>
        </div>
    </div>
</div>
    ...
    <div class="container">
        ...
        <div class="cash-payment" id="cash-payment">
            ...
            <div class="back-to-pos-cash" id="back-to-pos-cash">
                <button onclick="goBackToPOS(true)">Order Again</button>
            </div>
        </div>
    ...
    let currentPaymentCode = ''; // Global variable to store the selected payment code

    // Set the total in the display
    document.getElementById('checkout-total').textContent = totalAmount.toFixed(2);

    // Modal functions
    function showModal(type, title, message, buttonText = 'OK', onButtonClick = null) {
    ...
    function processPayment(code, type) {
        currentPaymentCode = code; // Store the specific payment code globally
    ...
    function calculateChange() {
        ...
        if (isNaN(cashReceived) || cashReceived < totalAmount) {
            showModal('error', 'Invalid Amount', 'Please enter an amount greater than or equal to the total.');
        } else {
            ...
            // Show success modal with change information
            showModal('success', 'Transaction Complete', `Change: ₱${change.toFixed(2)}`, 'OK', function() {
    ...
    function confirmPayment() {
        ...
        if (referenceNumber.length !== 4) {
            showModal('error', 'Invalid Reference', 'Please enter the last 4 digits of the reference number.');
            return;
        }
        ...
        showModal('success', 'Payment Confirmed!', `Reference Number: ${referenceNumber}`, 'OK', function() {
    ...
    function saveTransaction(paymentMethod, cashReceived = null, change = null, referenceNumber = null) {
        // Use the globally stored payment code instead of the generic type
        const specificPaymentCode = currentPaymentCode;

        const transactionData = {
            paymentMethod: specificPaymentCode, // Now this will be 'CASH', 'GCASH', 'MAYA' etc.
            total: totalAmount,
            cart: cartData,
        ...
        .then(data => {
            if (!data.success) {
                ...
                showModal('error', 'Transaction Error', 'Error saving transaction: ' + (data.error || 'Unknown error'));
            } else {
                ...
                let detailsHTML = `
                    <div class="transaction-info">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${specificPaymentCode}</span>
                        </div>
                        <div class="detail-row">
                ...
                detailsHTML += `</div>
                    ...
                    <button onclick="goBackToPOS(true)" class="print-button" style="margin-top: 10px; background-color: #6c5ce7;">New Order</button>`;
        ...
        .catch(error => {
            ...
            showModal('error', 'Network Error', 'Error saving transaction. Please try again.');
        });
    }
    ...
    function goBackToPOS(clearCart = false) {
        if (clearCart) {
            // Clear the cart and redirect
            localStorage.removeItem('savedCart');
            window.location.href = 'clear_cart.php';
        } else {
            // Save the current cart state before navigating
            if (cartData && Object.keys(cartData).length > 0) {
                saveCartState();
            }
